...
很多游戏佬，尤其是B站上众多游戏UP主，都会自己管理存档，以便于重复挑战Boss和录视频。对于这些已有备份，导入本应用的方式极其简单，直接将其移动或复制到本应用指定的目录下即可。
...
本项目首发[Gitee](https://gitee.com/wolray/nonlinear-backup)，使用GPL v2协议开源，在右侧[发行版页面](https://gitee.com/wolray/nonlinear-backup/releases/)点击最新版下载即可。在使用时可能会需要安全软件拦截，忽视即可。

你也可以安装[AHK v2.0](https://www.autohotkey.com/)，clone本项目到本地，然后自行编译 `backup.ahk` 即可得到 `backup.exe` 可执行程序。
